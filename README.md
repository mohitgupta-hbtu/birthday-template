...
Aapko code seekhne ya files me edits dhoondhne ki bilkul zaroorat nahi hai. Hum code edit karne ke liye humare smart AI coding assistant ka use karenge! Aap bas instructions flow follow kijiye:
...
3. **Cursor me open karein:** Apne computer par **Cursor / VS Code** editor ko open kijiye, aur iske terminal me copied link se repository clone execute kijiye:
...
> Agar aapke computer me **Node.js download nahi hai** ya commands run karte time koi bhi error (jaise name resolutions, missing dependencies, variables issues) aaye, toh bas terminal/chat me **AI assistant se kahein**:  
> *"Hey, please download Node.js and fix all command execution errors."*  
...
2. Us screenshot ko **AI / Cursor** chat widget me paste/attach karein.
3. AI Agent ko simple language me instructions dein:
    - *"Hey, please replace the name in this landing page screenshot with my friend 'Rohit'."*
...
* **How to edit:** Apne images ko clone directory me add karein aur AI coding assistant se kahein: *"Hey, please replace the hero image slider with my uploaded pictures"* or manual configurations `birthdayData.ts` me file import karke edit kar sakte hain.
...
* **How to edit:** AI assistant se terminal chat me plain words me boleya: *"Hey, custom modify main intro text: change name to Kabir and add a funny greeting card wishing him happy birthday"* or edit the `hero.introParas` text array in `birthdayData.ts` manually.
...
**Yahan par aap aapne first meeting spot ya kisi special place ki story share karein!** 🗺️
...
* **How to edit:** AI chat widget me explain karein: *"Hey, edit first story block: set imageAlt to 'Cafe Coffee Day' and change lines to describe how we first met there by accident"* or edit the first story item in `journey.storyBlocks` in `birthdayData.ts` in the workspace.
...
* **How to edit:** AI chat screen par brief karein: *"Hey, update first story block text description to write a sweet short paragraph about how we used to spend hours sitting and sharing memes"* or change the array layout values inside `journey.storyBlocks[0].lines` inside `birthdayData.ts` manually.
...
* **How to edit:** AI chat screen par specify karein: *"Hey, replace the second story block image with my holiday trip group photo"* or update the image path for the second item in `journey.storyBlocks[1].image` config array inside `birthdayData.ts` manually.
...
* **How to edit:** AI chat me instructions dein: *"Hey, rewrite the text inside the second story paragraph to make it a deep, emotional thank-you paragraph expressing how grateful I am for their friendship"* or change the text array value inside `journey.storyBlocks[1].lines` inside `birthdayData.ts` manually.
...
* **How to edit:** AI chat console me clear instuctions dein: *"Hey, connect my uploaded pictures to these 6 polaroid slots in the gallery page"* or update parameters under `gallery.images` config inside `birthdayData.ts` for quick placeholder naming, and hook actual assets using local files mapping.
...
* **How to edit:** AI chat board me simple input karein: *"Hey, change background music video ID to: [Copied YouTube Video ID] and put my uploaded photo in the center of the spinning vinyl record"* or update the `gallery.youtubeMusicId` text variable setting inside `birthdayData.ts` manually.
...
* **How to edit:** AI chat console me input karein: *"Hey, replace the finale card background photo with my uploaded picture"* or edit the files mapping and paths inside the workspace.
...
* **How to edit:** AI chat box me direction dein:
  * *"Hey, change the creator badge and footer to show 'YOUR LOVE' and 'made with ❤️ for my favorite person'."*
...
1. **AI Se Commit Karwayein:** Sabse pehle apni code/chat screen par AI assistant ko bol dein ki saare changes commit kar de. AI automatic aapke saare changes save aur commit kar dega.
